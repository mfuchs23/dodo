...
public class WebhelpGenerator extends Generator {

	public WebhelpGenerator() throws IOException {

		super();
		setTarget("dbdoclet.docbook.webhelp");
	}
	...
	public void run() {
		...
		try {
			...
			// tsFile = File.createTempFile("dbd-webhelp", ".ts");
			...
			File baseDir = getBaseDir(project.getDriver(Output.webhelp));
			...
			relocateHtml(baseDir, Output.webhelp);
			...
			messageListener.fatal("WebhelpGenerator", oops);
	...
	private File getIndexFile() throws IOException {
		...
		AbstractDriver driver = project.getDriver(Output.webhelp);
