<!--
 ...
 RCS INFORMATION
 Id.........: $Id: CrossOverTransform.xsl,v 1.1.1.1 2004/12/21 13:59:28 mfuchs Exp $
...
<xsl:stylesheet 
  ...
  <xsl:template match="element">
    ...
    <xsl:param name="file" select="concat(concat('java/org/dbdoclet/test/transform/CrossOver',$tagLabel),'Tests.java')"/>
    <redirect:write select="$file">
    <xsl:text>package org.dbdoclet.test.transform;
</xsl:text>

import junit.framework.Test;
import junit.framework.TestSuite;

public class CrossOver<xsl:value-of select="$tagLabel"/>Tests extends XmlTestCase 
{
...
<xsl:for-each select="/elements/element">
  ...
  <xsl:template name="write-test">
    ...
    public void testCrossOver<xsl:value-of select="$tag1Upper"/>In<xsl:value-of select="$tag2Upper"/>() 
    {
        ...
        checkDocBook(code,"<xsl:value-of select="$tag1Upper"/>");
    }
  </xsl:template>

</xsl:stylesheet>
<!--
$Log: CrossOverTransform.xsl,v $
