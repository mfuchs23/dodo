...
<xsl:stylesheet exclude-result-prefixes="#default" version="1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:param name='admon.graphics.path'>c:/Programme/DocBook Doclet//docbook/xsl/images/</xsl:param>
  <xsl:param name='callout.graphics'>1</xsl:param>
  <xsl:param name='callout.graphics.extension'>.gif</xsl:param>
  <xsl:param name='callout.graphics.path'>c:/Programme/DocBook Doclet//docbook/xsl/images/callouts/</xsl:param>
  ...
  <xsl:param name='draft.watermark.image'>c:/Programme/DocBook Doclet//docbook/xsl/images/draft.png</xsl:param>
  ...
  <xsl:template match="constructorsynopsis|destructorsynopsis|methodsynopsis" mode="java">
    ...
    <fo:block linefeed-treatment="preserve" white-space-collapse="false" wrap-option="wrap" xsl:use-attribute-sets="synopsis.properties">
      ...
      <xsl:apply-templates mode="java" select="methodparam">
        <xsl:with-param name="indent" select="string-length($decl)"></xsl:with-param>
        <xsl:with-param name="decl-length" select="$decl-length"></xsl:with-param>
      </xsl:apply-templates>
      <xsl:text>)</xsl:text>
...
           </xsl:text>
        ...
        <xsl:when test="$decl-length > 80">
          ...
          <xsl:if test="$indent > 0">
            <xsl:call-template name="copy-string">
              ...
              <xsl:with-param name="count" select="$indent + 1"></xsl:with-param>
            </xsl:call-template>
          </xsl:if>
        </xsl:when>
        <xsl:otherwise>
  ...
  <xsl:template name="get-decl-length">
    ...
    <xsl:value-of select="$indent + string-length($buffer)"></xsl:value-of>
  </xsl:template>
  ...
  <xsl:template match="modifier" mode="java">
    <xsl:choose>
      <xsl:when test="@role = 'annotation'"><fo:inline xsl:use-attribute-sets="synopsis.annotation"><xsl:apply-templates mode="java"></xsl:apply-templates></fo:inline></xsl:when>
      <xsl:otherwise>
        <fo:inline xsl:use-attribute-sets="synopsis.keyword">
          <xsl:apply-templates mode="java"></xsl:apply-templates>
        </fo:inline>
